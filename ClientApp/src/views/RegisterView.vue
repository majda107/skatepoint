<template>
  <div class="auth main-wrapper">
      <div class="auth-content">
          ...
          <input v-model="username" type="text" class="input-s mt-32 width-100p" placeholder="Uživatelské jméno" >
          <input v-model="username" type="text" class="input-s mt-24 width-100p" placeholder="priklad@mail.cz" >
          <input v-model="password" type="password"  class="input-s mt-24 width-100p" placeholder="Heslo">
          <button @click="this.login" class="btn-m btn-primary mt-24 width-100p">Registrovat</button>
      </div>
  </div>
</template>
